...
public struct PelotaSignalRequestClient: Sendable {
    public let configuration: PelotaSignalConfiguration
    public var session: URLSession

    public init(configuration: PelotaSignalConfiguration, session: URLSession? = nil) {
    ...
    public func loadDecision() async throws -> PelotaSignalDecision {
        try await loadDecision(preferredLanguage: Self.defaultLanguageCode())
    }

    public func loadDecision(preferredLanguage: String) async throws -> PelotaSignalDecision {
        ...
        return PelotaSignalDecision(
            enabled: true,
            url: Self.resolvedDestinationURL(base: url, preferredLanguage: preferredLanguage)
        )
    }

    public func makeLaunchRequest(payload: PelotaSignalLaunchPayload) throws -> URLRequest {
    ...
    public static func defaultLaunchPayload(
        configuration: PelotaSignalConfiguration,
        preferredLanguage: String = Locale.current.identifier
    ) -> PelotaSignalLaunchPayload {
        let bundle = Bundle.main
        return PelotaSignalLaunchPayload(
            event: "app_open",
    ...
    private func send(request: URLRequest) async throws -> PelotaSignalDecision {
        ...
        return try JSONDecoder().decode(PelotaSignalDecision.self, from: data)
    }
