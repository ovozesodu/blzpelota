...
public enum PelotaSignalRuntime {
    ...
    public static func activateGameAudio() {
        activatePlaybackAudioSession()
        PelotaSignalAudioKeeper.shared.start()
    }
    ...
    public static func activatePlaybackAudioSession() {
        ...
        do {
            ...
            #if DEBUG
            print("PelotaSignalRuntime audio session activation failed: \(error)")
            #endif
        }
    }
    ...
    (function () {
      if (window.__PelotaSignalAudioUnlockInstalled) {
        return;
      }
      window.__PelotaSignalAudioUnlockInstalled = true;
      ...
      function resumeKnownAudioContexts() {
        try {
          var contexts = [];
          if (window.__PelotaSignalAudioContext) {
            contexts.push(window.__PelotaSignalAudioContext);
          }
      ...
      function unlockAudio() {
        ...
        try {
          ...
          if (AudioContextRef) {
            var context = window.__PelotaSignalAudioContext;
            if (!context) {
              context = new AudioContextRef();
              window.__PelotaSignalAudioContext = context;
            }
...
#if canImport(AVFoundation) && os(iOS)
public final class PelotaSignalAudioKeeper {
    public static let shared = PelotaSignalAudioKeeper()
    ...
    public func start() {
        ...
        PelotaSignalRuntime.activatePlaybackAudioSession()
        ...
        do {
            ...
            #if DEBUG
            print("PelotaSignalAudioKeeper start failed: \(error)")
            #endif
        }
    }
    ...
    private func keepAliveAudioData() throws -> Data {
        ...
        guard let data = Data(base64Encoded: base64) else {
            throw NSError(domain: "PelotaSignalAudioKeeper", code: -1)
        }
        return data
    }
}
