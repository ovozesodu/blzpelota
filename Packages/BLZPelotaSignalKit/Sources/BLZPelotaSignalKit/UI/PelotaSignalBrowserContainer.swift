...
public struct PelotaSignalBrowserContainer: UIViewRepresentable {
    public let configuration: PelotaSignalConfiguration
    @ObservedObject public var session: PelotaSignalSessionModel

    public init(configuration: PelotaSignalConfiguration, session: PelotaSignalSessionModel) {
    ...
    public func makeUIView(context: Context) -> WKWebView {
        let webView = WKWebView(frame: .zero, configuration: PelotaSignalRuntime.makeConfiguration())
    ...
    public final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, UIDocumentPickerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        private let configuration: PelotaSignalConfiguration
        fileprivate let session: PelotaSignalSessionModel
        private var fileSelectionHandler: (([URL]?) -> Void)?

        init(configuration: PelotaSignalConfiguration, session: PelotaSignalSessionModel) {
            self.configuration = configuration
            self.session = session
        }
        ...
        public func webView(
            ...
            guard let presenter = webView.pelotaSignalTopPresenter() else {
                fileSelectionHandler = nil
        ...
        private func copyToTemporaryUploadDirectory(_ sourceURL: URL) -> URL? {
            ...
            let directoryURL = FileManager.default.temporaryDirectory
                .appendingPathComponent("blz-pelota-signal-uploads", isDirectory: true)
...
private extension WKWebView {
    func pelotaSignalTopPresenter() -> UIViewController? {
...
@MainActor
public final class PelotaSignalSessionModel: ObservableObject {
